...
const { Intermidiate_Exam_Regisration } = require("../models");
const {validateToken} = require('../middlewares/AuthMiddlewear')


router.get("/", async (req, res) => {
  const listOfpayment = await Intermidiate_Exam_Regisration.findAll();
...
router.post("/",validateToken, async (req, res) => {
  ...
  const createdRecord = await Intermidiate_Exam_Regisration.create({
    RegNo: id,
...
router.get("/user",validateToken, async (req, res) => {
  const id = req.user.RegNo
  try {
    const payment = await Intermidiate_Exam_Regisration.findOne({
      where: { RegNo : id} 
    });
