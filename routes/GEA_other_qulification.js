const express = require("express");
const router = express.Router();
const { GEA_other_qulification } = require("../models");
const { validateToken } = require("../middlewares/AuthMiddlewearApplicant");

router.get("/", validateToken, async (req, res) => {
    const listOfotherqulification = await GEA_other_qulification.findAll();
    res.json(listOfotherqulification);
});

router.post("/", validateToken, async (req, res) => {
    const post = req.body;
    await GEA_other_qulification.create(post);
    res.json(post);
});

router.get("/:id",validateToken, async (req, res) => {
    const id = req.params.id; 
    const otherqulification = await GEA_other_qulification.findByPk(id);
    res.json(otherqulification);
})

router.put("/:id", validateToken, async (req, res) => {
    try {
      const id = req.params.id;
      const body = req.body;
  
      const existingApplicant = await GEA_other_qulification.findOne({
        where: { GEApplicantID: id }
      });
  
      if (existingApplicant) {

        await GEA_other_qulification.update(body, { where: { GEApplicantID: id } });
        return res.status(200).json({ message: "Applicant edit request updated successfully" });
      } else {

        await GEA_other_qulification.create({ GEApplicantID: id, ...body});
        return res.status(200).json({ message: "New applicant record created successfully" });
      }
  
    } catch (error) {
      res.status(500).json({ error: error.message || "Failed to update applicant edit request" });
    }
  });


module.exports = router;