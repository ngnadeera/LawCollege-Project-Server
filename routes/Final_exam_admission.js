...
const { Final_exam_admission } = require("../models");
const {validateToken} = require('../middlewares/AuthMiddlewear')


router.get("/", async (req, res) => {
  const listOfpayment = await Final_exam_admission.findAll();
...
router.post("/",validateToken, async (req, res) => {
  ...
  const createdRecord = await Final_exam_admission.create({
    RegNo: id,
    ...postData
  });
...
router.get("/:index",validateToken, async (req, res) => {
  ...
  try {
    const payment = await Final_exam_admission.findAll({
      where: { indexNo : index} 
    });
...
router.put("/:id", async (req, res) => {
  ...
  try {
    const payment = await Final_exam_admission.findOne({
      where: { RegNo: id },
    });
    if (payment) {
