...
const { Preliminary_Exam_Regisration } = require("../models");
const {validateToken} = require('../middlewares/AuthMiddlewear')


router.get("/", async (req, res) => {
  const listOfpayment = await Preliminary_Exam_Regisration.findAll();
...
router.post("/",validateToken, async (req, res) => {
  ...
  const createdRecord = await Preliminary_Exam_Regisration.create({
    RegNo: id,
...
//   try {
//     const payment = await Preliminary_Exam_Regisration.findOne({
//       where: { RegNo: id },
//     });
//     if (payment) {
//       res.json(payment);
