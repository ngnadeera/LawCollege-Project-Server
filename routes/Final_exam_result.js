...
const { Final_exam_results } = require("../models");
...
router.get("/", validateToken, async (req, res) => {
  ...
  try {
    const listOfResults = await Final_exam_results.findAll({
      where : {RegNo:id},
    });
...
router.get("/:id", async (req, res) => {
  ...
  try {
    const result = await Final_exam_results.findOne({
      where: {IndexNo:id},
    });
    res.json(result);
  } catch (error) {
...
//     await Final_exam_results.bulkCreate(results);
...
router.post("/", async (req, res) => {
  ...
  try {
    const createdRecord = await Final_exam_results.create(postData);
...
router.delete("/", async (req, res) => {
  try {
    await Final_exam_results.destroy({ where: {} });
    res.json({
